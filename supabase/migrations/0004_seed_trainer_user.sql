-- 0004_seed_trainer_user.sql
-- Creates a trainer account directly in auth.users, then links it to the
-- 'trainer' role. Run AFTER 0001. Safe to re-run — it no-ops if the user exists.
--
...
do $$
declare
  v_email    text := 'rafi@gmail.com';
  v_password text := 'rafi1234';
  v_name     text := 'Rafi';
  v_user_id  uuid;
begin
  ...
  -- Link the user to the 'trainer' role via the relation table.
  insert into public.user_role_assignments (user_id, role_id)
  select v_user_id, r.id from public.user_roles r where r.name = 'trainer'
